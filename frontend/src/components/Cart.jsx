...
const Cart = ({ onCheckout }) => {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-sm z-50 flex flex-col shadow-2xl slide-in"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
          {items.length === 0 ? (
            ...
            <div key={item._id} className="rounded-xl p-3 flex gap-3"
              style={{ background: '#F8FAEE', border: '1px solid rgba(214,153,60,0.18)', boxShadow: '0 2px 8px rgba(50,88,98,0.06)' }}>
              <div className="flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-1.5">
                    <div className={item.veg ? 'veg-icon' : 'nonveg-icon'} />
                    <p className="font-semibold text-sm leading-tight" style={{ color: '#1a1a1a', fontFamily: 'Poppins,sans-serif' }}>{item.name}</p>
                  </div>
                  <button onClick={() => removeItem(item._id)} className="text-red-700 hover:text-red-600 transition-colors flex-shrink-0">
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
                      <path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/>
                    </svg>
                  </button>
                </div>
                <p className="text-xs mt-0.5" style={{ color: '#64690C', fontFamily: 'Poppins,sans-serif' }}>₹{item.price} each</p>
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-1.5 rounded-full px-1 py-0.5"
                    ...
                    <button onClick={() => decreaseQty(item._id)}
                      ...
                      style={{ color: '#325862' }}>−</button>
                    <span className="text-white font-bold text-sm min-w-[20px] text-center"
                      style={{ fontFamily: 'Poppins,sans-serif' }}>{item.quantity}</span>
        ...
        {items.length > 0 && (
          <div className="px-5 py-4 space-y-3"
            ...
            <div className="flex items-center justify-between">
              <span className="font-semibold text-sm" style={{ color: '#325862', fontFamily: 'Poppins,sans-serif' }}>Subtotal</span>
              <span className="font-black text-lg" style={{ color: '#31603D', fontFamily: 'Poppins,sans-serif' }}>₹{totalAmount}</span>
            </div>
            <button
